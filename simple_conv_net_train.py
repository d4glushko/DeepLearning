...
class SimpleConvNet(nn.Module):
    def __init__(self, device, nn_type: NNType):
    ...
    def forward(self, x):
        if self.nn_type == NNType.TORCH:
            y = self.__forward_torch(x)
        elif self.nn_type == NNType.SCALAR:
            y = self.__forward_scalar(x)
        elif self.nn_type == NNType.VECTOR:
            y = self.__forward_vector(x)
        return y

    def __forward_torch(self, x):
...
def main(args):
    ...
    model = SimpleConvNet(device, NNType(args.nn_type))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--nn-type', type=int, default=NNType.TORCH.value, metavar='N', 
        help='choose type of nn: {} - {}, {} - {}, {} - {} (default: {})'.format(
            NNType.TORCH.value, NNType.TORCH.name, NNType.SCALAR.value, NNType.SCALAR.name, 
            NNType.VECTOR.value, NNType.VECTOR.name, NNType.TORCH.name
        )
    )
